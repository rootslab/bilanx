/*
 * ♎ Bilanx, module to handle bindings between commands and Redis replies,
...
exports.Bilanx = ( function () {
    ...
    bproto.push = function ( ocmd ) {
        var me = this
            ...
            , cname = ocmd.cmd
            , isQuit = ocmd.isQuit
            , isSub = ocmd.isSubscription
            , err = null
            ;
        if ( ! isQuit ) {
            if ( m.on ) return sendError( ocmd, 'only the QUIT command is allowed in monitor mode.' );
            else if ( s.on && ! isSub ) return sendError( ocmd, ocmd.cmd + ' command is not allowed in PubSub mode.' );
        }
